...
vec3 Render_World::Cast_Ray(const Ray& ray, int recursion_depth) const
{
    ...
    if (cast.second.dist >= small_t){
        ...
        color = cast.first.shader->Shade_Surface(*this, ray, cast.second, location, normal, recursion_depth);
    }else{
        Pixel_Print("use background_shader");
        if (background_shader != nullptr){
            if (cast.first.object != nullptr){
                color = background_shader->Shade_Surface(*this, ray, cast.second, vec3(0, 0, 0), cast.first.object->Normal(ray, cast.second), recursion_depth);
            }else{
                color = background_shader->Shade_Surface(*this, ray, cast.second, vec3(0, 0, 0), vec3(0, 1, 0), recursion_depth);
            }
        }else{
            color = vec3(0, 0, 0);
        }
