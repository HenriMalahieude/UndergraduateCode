...
{
    vec3 result = film_position - (0.5 * image_size[0] * horizontal_vector) - (0.5 * image_size[1] * vertical_vector); //Determine bottom left corner in space
    vec3 vert_pixed = vertical_vector * pixel_size[1]; //vertical distance of one pixel
    vec3 hori_pixed = horizontal_vector * pixel_size[0]; //horizontal distance of one pixel
    
    result = result + ((pixel_index[0] + 0.5) * hori_pixed) + ((pixel_index[1] + 0.5) * vert_pixed);
