    ...
    const vec3& intersection_point,const vec3& normal,int recursion_depth) const
{
    ...
    for (size_t i = 0; i < render_world.lights.size(); i++){
        ...
        if ((render_world.enable_shadows && can_get_color) || !render_world.enable_shadows){
            ...
            Pixel_Print("shading for light L: diffuse: (", local_diffuse[0]," ", local_diffuse[1]," ", local_diffuse[2],"); specular: (", local_specular[0]," ", local_specular[1]," ", local_specular[2],")");
        }
    }

    vec3 color = (ambient_component + diffuse_component + specular_component);
    Pixel_Print("final color (", color[0]," ", color[1]," ", color[2], ")");
    return color;
}
