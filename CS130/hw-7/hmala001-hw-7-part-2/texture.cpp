...
{
    int i = std::floor(uv[0] * width);
    int j = std::floor(uv[1] * height);

    i = wrap(i, width);
    j = wrap(j, height);

    Pixel nearestNeighbor = data[j * (width) + i];
    vec3 cc = From_Pixel(nearestNeighbor);

    Pixel_Print("texture (u,v): ", uv,"  (i,j): ", i," ", j,"  color: ", cc);
